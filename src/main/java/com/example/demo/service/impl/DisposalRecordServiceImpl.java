...
public class DisposalRecordServiceImpl implements DisposalRecordService {
    ...
    public DisposalRecord createDisposal(Long assetId, DisposalRecord disposal) {
        ...
        User admin = userRepository.findById(disposal.getApprovedBy().getId()).orElseThrow(() -> new ResourceNotFoundException("User not found"));
        
        if (!"ADMIN".equals(admin.getRole())) throw new ValidationException("Approver must be admin");
        if (disposal.getDisposalDate().isAfter(LocalDate.now())) throw new ValidationException("future disposal date");
